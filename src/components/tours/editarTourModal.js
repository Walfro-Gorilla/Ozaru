...
const EditarTourModal = (props) => {
  ...
  const [tourDescripcion, setTourDescripcion] = useState(datosTour || tourDescripcionModel);
  ...
  const onSubmit = () => {
    let tourRef = props.firebase.firestore.collection("TourDescripciones").doc(docId);

    // Set de data
    return tourRef.update(tourDescripcion)
      .then(function () {
...
export default withFirebase(EditarTourModal);